...
export module soltsice {
  ...
  function parseArgs(
    args: SoltsiceOptions
  ): { source: string; destination: string; W3importPath: string; cleanArtifacts: boolean; skipPattern: string } {
  ...
  function getDestinationPaths(destination: string, files: string[]): string[] {
    let filesArr = files
      .map(f => path.resolve(path.join(destination, f))) // __dirname,
  ...
  function processAbi(abi: W3.ABIDefinition): string {
    ...
    let methodsBody: string = isConstant
      ? `
        ...
        sendTransaction: Object.assign((${
          inputsString === '' ? '' : inputsString + ','
            ...
            },
            {
              ...
              sendSigned: (${
                inputsString === '' ? '' : inputsString + ','
              } privateKey: string, txParams?: W3.TX.TxParams, nonce?: number): Promise<string> => {
                // tslint:disable-next-line:max-line-length
  ...
  function processFile(fileName: string, filePath: string, targetPath: string, W3ImportPath: string): string {
    ...
    // When a contract is created, its constructor (a function declared with the constructor keyword)
...
export class ${contractName} extends SoltsiceContract {
  ...
  // tslint:disable-next-line:max-line-length
  public static async new(deploymentParams: W3.TX.TxParams, ctorParams?: {${
    ctorParams.typesNames
  }}, w3?: W3, link?: SoltsiceContract[], privateKey?: string): Promise<${contractName}> {
    w3 = w3 || W3.default;
    if (!privateKey) {
